...
extends HUDComponent {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        this.width = Minecraft.getMinecraft().fontRenderer.getStringWidth("XP: " + InventoryUtil.getTotalAmountOfItem(Items.EXPERIENCE_BOTTLE) + (GeraldHack.INSTANCE.moduleManager.getModule(XPCount.class).stackCount.getValue() ? " [" + (float)InventoryUtil.getTotalAmountOfItem(Items.EXPERIENCE_BOTTLE) / 64.0f + "]" : ""));
        this.height = Minecraft.getMinecraft().fontRenderer.FONT_HEIGHT;
        Minecraft.getMinecraft().fontRenderer.drawStringWithShadow("XP: " + (Object)ChatFormatting.GRAY + InventoryUtil.getTotalAmountOfItem(Items.EXPERIENCE_BOTTLE) + (GeraldHack.INSTANCE.moduleManager.getModule(XPCount.class).stackCount.getValue() ? " [" + MathHelper.ceil((float)((float)InventoryUtil.getTotalAmountOfItem(Items.EXPERIENCE_BOTTLE) / 64.0f)) + "]" : ""), (float)this.x, (float)this.y, new Color((int)GeraldHack.INSTANCE.moduleManager.getModule(ClickGui.class).color.getR(), (int)GeraldHack.INSTANCE.moduleManager.getModule(ClickGui.class).color.getG(), (int)GeraldHack.INSTANCE.moduleManager.getModule(ClickGui.class).color.getB()).getRGB());
    }
    ...
    public void mouseReleased(int mouseX, int mouseY, int mouseButton) {
        GeraldHack.INSTANCE.moduleManager.getModule(XPCount.class).x.setValue(this.x);
        GeraldHack.INSTANCE.moduleManager.getModule(XPCount.class).y.setValue(this.y);
        this.stopDragging();
    }
